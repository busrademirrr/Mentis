...
export const ChatView = ({ user }: { user: any }) => {
    ...
    const handleMenuAction = (action: string) => {
        ...
        setTimeout(() => {
            if (action === 'clear') {
                setMessages([]);
                Toast.show({ type: 'success', text1: 'Sohbet temizlendi' });
            } else if (action === 'delete') {
                navigation.goBack();
                Toast.show({ type: 'success', text1: 'Sohbet silindi' });
            } else if (action === 'mute') {
                Toast.show({ type: 'info', text1: 'Bildirimler sessize alındı' });
            } else if (action === 'block') {
                Toast.show({ type: 'info', text1: 'Kullanıcı engellendi' });
                navigation.goBack();
            }
        }, 500);
    };
        ...
        <SafeAreaView style={styles.safeArea}>
            ...
            <View style={styles.header}>
                ...
                <View style={styles.headerUserInfo}>
                    ...
                    <View>
                        <Text style={styles.headerUserName}>{user.name || user.username || 'Bilinmeyen Kullanıcı'}</Text>
                        {isTyping ? (
                ...
                <TouchableOpacity style={styles.infoBtn} onPress={() => setMenuVisible(true)}>
            ...
            <Modal visible={isMenuVisible} transparent animationType="slide">
                <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setMenuVisible(false)}>
                    <View style={styles.modalContent} onStartShouldSetResponder={() => true}>
                        ...
                        <TouchableOpacity style={styles.menuItem} onPress={() => handleMenuAction('mute')}>
                            <Icon name="bell-off" size={20} color={colors.textPrimary} />
                            <Text style={styles.menuItemText}>Sessize Al</Text>
                        </TouchableOpacity>

                        <TouchableOpacity style={styles.menuItem} onPress={() => handleMenuAction('clear')}>
                            <Icon name="trash-2" size={20} color={colors.textPrimary} />
                            <Text style={styles.menuItemText}>Sohbeti Temizle</Text>
                        </TouchableOpacity>
                        ...
                        <TouchableOpacity style={styles.menuItem} onPress={() => handleMenuAction('block')}>
                            <Icon name="shield-off" size={20} color={colors.error} />
                            <Text style={[styles.menuItemText, { color: colors.error }]}>Kullanıcıyı Engelle</Text>
                        </TouchableOpacity>
                    </View>
                </TouchableOpacity>
            </Modal>
...
export const ChatScreen = () => {
    ...
    const { user } = route.params || { user: { name: 'Bilinmeyen Kullanıcı', avatar: 'https://images.unsplash.com/photo-1543857778-c4a1a3e0b2eb' } };

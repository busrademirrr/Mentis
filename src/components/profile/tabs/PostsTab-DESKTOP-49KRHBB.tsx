...
export const PostsTab: React.FC<PostsTabProps> = ({ posts }) => {
    ...
    const renderItem = ({ item }: { item: any }) => {
        switch (item.type) {
            case 'discussion':
                return <DiscussionCard post={item} onVote={() => {}} onDelete={() => handleOptimisticDelete(item.id)} />;
            case 'quiz':
                return <QuizCard post={item} onAnswer={() => {}} onDelete={() => handleOptimisticDelete(item.id)} />;
            case 'article':
            case 'hero':
            default:
                return <KnowledgeCard post={item} onDelete={() => handleOptimisticDelete(item.id)} />;
        }
    };
            ...
            <View style={styles.emptyContainer}>
                ...
                <View style={styles.emptyActions}>
                    <TouchableOpacity style={styles.actionBtnPrimary} onPress={() => navigation.navigate('CreateCard')}>
                        ...
                        <Text variant="label" weight="bold" color="surface" style={{ marginLeft: 8 }}>İlk Gönderini Oluştur</Text>
                    </TouchableOpacity>
                </View>
            </View>
        );

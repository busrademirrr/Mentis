...
// Mock data to simulate Supabase architecture
export const mockHeroSociety: Society = {
    id: '1',
...
export const mockLiveDebates: LiveDebate[] = [
    {
        id: 'd3',
        society_id: '3',
        title: 'Yapay zeka sanat üretebilir mi?',
...
export const societyService = {
    getHeroSociety: async (): Promise<Society> => {
        return new Promise(resolve => setTimeout(() => resolve(mockHeroSociety), 300));
    },
    
    getSocieties: async (category?: string): Promise<Society[]> => {
        return new Promise(resolve => setTimeout(() => resolve(mockSocieties), 400));
    },

    getLiveDebatesStream: async (): Promise<LiveDebate[]> => {
        return new Promise(resolve => setTimeout(() => resolve(mockLiveDebates), 350));
    }
};
